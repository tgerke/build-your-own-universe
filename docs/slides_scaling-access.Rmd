...
```{css css-focus-sliding-bg-1, echo=FALSE}
.focus-ul {
...
function connectElements(i, startSocket = 'top', endSocket = 'auto') {
  let startElements = document
    .querySelectorAll(`.remark-slides-area .leader-start-${i}`)
    
  startElements.forEach(function(start) {
    let end = start.closest('.remark-slide-content').querySelector(`.leader-end-${i}`)
...
function showOrDrawLineElement(el) {
  ...
  let lineElements = el.closest('.remark-slides-area').querySelectorAll('.' + lineClass)
  el.line.position()
...
```{css expand-in, echo=FALSE}
.expand-in {
  max-height: 0;
  overflow: hidden;
}
.remark-fading .expand-in {
  max-height: 600px;
}
.remark-visible .expand-in {
  animation: expand-in 1.5s linear;
  animation-fill-mode: forwards;
}
@keyframes expand-in {
  0% { max-height: 0 }
